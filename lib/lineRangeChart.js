...
(function($, window) {
    ...
    let DriverChartModule = {
        ...
        alignMarkers: function(options) { // filter1,filter2,filter3
            ...
            for (let i=0; i<options.subjects.length; i++) {
                ...
                for(let marker in options.markers) {
                    ...
                    let markerDom = $(DriverChartModule.chartDom).find('.'+marker+'.head-marker').eq(i);

                    if (typeof value != "undefined" && typeof value[marker] != "undefined") {

                        let left = width * value[marker] / options.max - 6.5;

                        let title = options.markers[marker].title+": "+value[marker];

                        markerDom.css('left', left).attr("title", title).addClass("bs-tooltip");
                    }else{
                        markerDom.css('display', 'none');
                    }
                }
            }
        },
            ...
            for (let i = 0; i < options.subjects.length; i++) {
                ...
                for(let j=0; j<subject.values.length; j++) {
                    ...
                    for(let marker in options.markers) {
                        ...
                        let markerDom = $(DriverChartModule.chartDom).find('.driver-row').eq(i).find('.'+marker+'.desc-marker').eq(j);
        ...
        connectTheDots: function(jQueryDotsContainer) {
            ...
            for(let marker in DriverChartModule.options.markers) {
                markers[marker] = $(jQueryDotsContainer).find('.'+marker+'.desc-marker');
            }

            let lineDom, i, Dy, Dx, length, angle;
            ...
            for(let marker in DriverChartModule.options.markers) {
                markers[marker] = $(jQueryDotsContainer).find('.'+marker+'.desc-marker .line').css("border-bottom-color", DriverChartModule.options.markers[marker].color);
            }
        },
        options: [],
        chartDom: {},
        colors: ["#fc5c65", "#4b6584", "#fd9644", "#778ca3", "#fed330", "#a55eea", "#26de81", "#45aaf2", "#4b7bec"]
    };
    ...
    function initColors() {
        ...
        for(let marker in DriverChartModule.options.markers) {
            ...
            let color = DriverChartModule.options.markers[marker].color;

            $(DriverChartModule.chartDom).find('.checkbox-form input[type=checkbox].'+marker+' + label').css("borderColor", color);

            $(DriverChartModule.chartDom).find('.'+marker+'.head-marker').css("borderColor", color);

            $(DriverChartModule.chartDom).find('.'+marker+'.desc-marker').css("backgroundColor", color);
        }
    ...
    function appendStaticDom(chartDom) {
        ...
        for(let marker in DriverChartModule.options.markers) {
            ...
            content += '<li><input checked id="filter-'+marker+'" class="'+marker+'" type="checkbox" /><label for="filter-'+marker+'">'+DriverChartModule.options.markers[marker].title+'</label></li>';
        }
    ...
    $.fn.lineRangeChart = function(options) {
        ...
        let i = 0;
        for(let marker in options.markers) {
            if (typeof options.markers[marker] == "string") {
                options.markers[marker] = {
                    title: options.markers[marker],
                    color: DriverChartModule.colors[i]
                };
                i++;
            }
            else if (options.markers[marker].color == "undefined") {
                options.markers[marker].color = DriverChartModule.colors[i];
                i++;
            }
        }
